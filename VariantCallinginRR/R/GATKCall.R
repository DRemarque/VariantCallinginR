...
#' @param gvcf.mode Whether to store the variant call data in a genomic file (g.vcf). Use this when calling variants from multiple sample alignments that are to be merged later into one large vcf. [default=FALSE]
#' @param BAM_Validation Whether to check for anomalies in the alignment file before performing variant calling. [default=FALSE]
#' @param BAM_Repair Whether to repair or replace the alignment file header. Enable this if the .BAM file is malformed or if the sample name is incorrect by inserting the new header as a character. An example of such a header is "-SM Example -LB 1 -PL Illumina -PU 10578252" [default=FALSE]
...
#' @param pathtoSamtools Only for running GATK on linux when BamValidation=T or BamRepair=T: the file path to the samtools software when a custom installation path has been used. The default is ./samtools, which assumes samtools is added to your $PATH.
...
#' @examples
#' # Simple call
#' GATKCall(Reference_Genome="ref.fasta",Alignment_File_Name="my_alignment.bam",Output_Base_Name="results")

GATKCall<-function(Reference_Genome="No Default", Alignment_File_Name="No Default", Output_Base_Name="Gatk_Call", gvcf.mode=FALSE, BAM_Validation=FALSE, BAM_Repair=FALSE, pathtobash="C:/Program Files/Git/bin/bash.exe",pathtoGATK="./gatk",pathtoSamtools="./samtools"){
  # Check Input
