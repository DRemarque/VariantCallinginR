...
#' @param pathtoBWA Only for running BWA on linux: the file path to the BWA software when a custom installation path has been used. The default is bwa, which assumes gatk is added to your $PATH.
...
#' @export
#' @examples
#' # Basic single end alignment for windows
#' BWAMEM_Align(Reference_Genome="ref.fasta",Forward_Fastq="reads.fastq",Paired_End_Data=F,Output_Base_Name="my_alignment")
#' # Paired end alignment with custom short flag
#' BWAMEM_Align(Reference_Genome="ref.fasta",Forward_Fastq="fw.fastq",Reverse_Fastq="rv.fastq",Paired_End_Data=T,Output_Base_Name="my_alignment",flag=""--rg-id 1 --rg SM:kip)

BWAMEM_Align<-function(Reference_Genome="No Default", Forward_Fastq="No Default", Reverse_Fastq="No Default", Paired_End_Data=TRUE, Output_Base_Name="Bowtie2_Alignment", pathtobash="C:/Program Files/Git/bin/bash.exe",pathtoBWA="bwa",pathtoGATK="./gatk", flag="--rg-id 1 --rg SM:Sample --rg LB:1 --rg PI:400 --rg PL:ILLUMINA"){
  # Check Input
